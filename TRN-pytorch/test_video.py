...
def pretrain_model_from_path(model, path):
    print(f'Loading old model from {path}')
    old_model_dict = torch.load(path)
    model_dict = model.state_dict()

    pretrained_state = { k:v for k,v in old_model_dict.items() if k in model_dict and v.size() == model_dict[k].size() }
    print(f'Old model dict has {len(pretrained_state.keys())} keys')
...
def get_predictions_for_8_frames(net, frames):
    ...
    input = data.view(-1, 3, data.size(1), data.size(2)).unsqueeze(0).half().cuda()
    with torch.no_grad():
        logits = net(input)
